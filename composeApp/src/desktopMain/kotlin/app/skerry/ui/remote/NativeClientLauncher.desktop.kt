...
private fun launchLinux(isRdp: Boolean, isVnc: Boolean, targetAddress: String, username: String, displayName: String) {
    ...
    if (remminaFile.exists() && tryStartProcess(listOf("remmina", "-c", remminaFile.absolutePath))) {
        return
    }
    ...
    if (isRdp) {
        ...
        if (tryStartProcess(args)) {
            return
        }
    } else if (isVnc) {
        if (tryStartProcess(listOf("vncviewer", targetAddress))) {
            return
        }
    }

    if (remminaFile.exists()) {
        tryStartProcess(listOf("xdg-open", remminaFile.absolutePath))
    }
}
...
private fun launchWindows(isRdp: Boolean, isVnc: Boolean, targetAddress: String, username: String, displayName: String) {
    if (isRdp) {
        ...
        try {
            ...
            if (tryStartProcess(listOf("mstsc.exe", rdpFile.absolutePath))) {
                return
            }
        } catch (_: Exception) {
        }
        tryStartProcess(listOf("mstsc.exe", "/v:$targetAddress"))
    } else if (isVnc) {
        tryStartProcess(listOf("vncviewer.exe", targetAddress))
    }
}
...
private fun launchMac(isRdp: Boolean, isVnc: Boolean, targetAddress: String, username: String) {
    if (isRdp) {
        ...
        tryStartProcess(listOf("open", rdpUri))
    } else if (isVnc) {
        tryStartProcess(listOf("open", "vnc://$targetAddress"))
    }
}

private fun tryStartProcess(cmd: List<String>): Boolean {
    return try {
