...
# jpackage's dependency probe walks the whole system library chain and writes it into Depends,
# including the libxdmcp → libbsd → libmd transitive tail. Kylin V10 ships libbsd0 but its repo
# has no libmd0, so dpkg -i aborts on a missing package the app never loads (readelf: no .so in
# the package has a NEEDED entry for libmd). Drop it from Depends.
sed -i -E 's/, ?libmd0//g' "$TMP/DEBIAN/control"
...
# ── 8. Repack ─────────────────────────────────────────────────────────
# -Zxz: 信创老系统（UOS/麒麟 V10）的 dpkg 不识别新版默认的 zstd 压缩
#（报 "unknown compression"），xz 是 dpkg 1.16+ 全支持的老格式。
dpkg-deb --root-owner-group -Zxz -b "$TMP" "$DST"
rm -rf "$TMP"
